...
void loop() {
  ...
  // Event loop
  for(i = 0; i < SS_MAX_CONNECTIONS; i++){
    if (connections[i].localState != LOCAL_RESET && local[i] && local[i].connected()) {
      ...
      if (local[i].available()) {
        ...
        if (connections[i].localState == LOCAL_COMPLETE) {
          ...
          if (connections[i].localState == LOCAL_AWAIT_GREETING) {
            ...
            if (connections[i].hostType == ADDR_TYPE_DOMAIN) {
            ...
            } else if (connections[i].hostType == ADDR_TYPE_IPV4) {
              if (local[i].available() >= 6) {
                local[i].readBytes(connections[i].host, 4);
                connections[i].port = local[i].read() * 256 + local[i].read();
                connections[i].localState = LOCAL_READY;
              } else {
          ...
          if (connections[i].localState == LOCAL_READY) {
            bool success;
            if (connections[i].hostType == ADDR_TYPE_DOMAIN) {
              r_log((char*) connections[i].host, connections[i].port, i);
              success = remote[i].connect(
                (char*) connections[i].host,
                connections[i].port);
            } else if (connections[i].hostType == ADDR_TYPE_IPV4) {
              byte ip[4];
              memcpy(ip, connections[i].host, 4);
              r_log("", connections[i].port, i);
              success = remote[i].connect(
                ip,
                connections[i].port);
            }
